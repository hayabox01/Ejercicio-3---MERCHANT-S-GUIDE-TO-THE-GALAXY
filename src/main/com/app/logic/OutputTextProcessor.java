...
public class OutputTextProcessor extends InputTextProcessor {
	...
	public static void processReplyForQuestion(){
		Map<String, String> map = questionAndAnswer;
	...
	public static void findValueOfRoman(String query){
		if (isValidinput(query)== true){
			...
			for (int i = 0; i < tokenValue.size(); i++) {
				tokenValueToRoman.add(tokenRomanMapping.get(tokenValue.get(i)));
			}
	...
	private static void findValueOfElement(String query){
		if (isValidinput(query) == true){
			...
			String element = null;
			for (int i = 0; i < tokenValue.size(); i++) {
				if(tokenRomanMapping.get(tokenValue.get(i)) != null){
					tokenValueToRoman.add(tokenRomanMapping.get(tokenValue.get(i)));
				}
				else if (ListElementValues.get(tokenValue.get(i)) != null){
			...
			float elementValue = (new RomanToDecimal().romanToDecimal(tokenValueToRoman.toString()) * ListElementValues.get(element));
