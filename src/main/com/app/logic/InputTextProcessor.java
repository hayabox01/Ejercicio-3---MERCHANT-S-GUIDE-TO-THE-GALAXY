...
public class InputTextProcessor {
	 ...
	 * questionAndAnswer :{how much is pish tegj glob glob ?, how many Credits is glob prok Iron ?}
	 ...
	 */
	static Map<String, String> tokenRomanMapping = new HashMap<String, String>();
	static Map<String, Float> tokenIntegerValue = new HashMap<String, Float>();
	//{pish=10.0, tegj=50.0, prok=5.0, glob=1.0}
	static Map<String, String> questionAndAnswer = new HashMap<String, String>();
	...
	static Map<String, Float> ListElementValues = new HashMap<String, Float>();
	...
	public static void ProcessFile(String filePath) throws IOException {
		...
		if (filePath == null){
			InputStream in = InputTextProcessor.class.getResourceAsStream("Input");
	...
	public static void processLine(String line){
		...
		if (line.endsWith("?")){
			questionAndAnswer.put(line,"");
		}
		else if (arr.length == 3 && arr[1].equalsIgnoreCase("is")){
			tokenRomanMapping.put(arr[0], arr[arr.length-1]);
		}
	...
	public static void MapTokentoIntegerValue(){
		...
		Iterator it = tokenRomanMapping.entrySet().iterator();
		while (it.hasNext()) {
	...
	private static void deCodeMissingQuery(String query){
		...
		for (int j = 0; j < valueofElement.length; j++) {
			stringBuilder.append(tokenRomanMapping.get(valueofElement[j]));
		}
		float valueOfElementInDecimal = new RomanToDecimal().romanToDecimal(stringBuilder.toString());
		ListElementValues.put(element, creditValue/valueOfElementInDecimal);
	}
